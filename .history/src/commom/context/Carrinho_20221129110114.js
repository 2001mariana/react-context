...
export const useCarrinhoContext = () => {
    ...
    function removerProduto(idProduto) {
        ...
        const produtoNoCarrinhoIgualZero = carrinho.filter(itemDoCarrinho => itemDoCarrinho.id === idProduto && itemDoCarrinho.quantidade === 0)
    
        if(!temOProduto) {    
          return setCarrinho(carrinhoAnterior => carrinhoAnterior.filter(itemDoCarrinho => itemDoCarrinho.id !== idProduto))
        }

        if(produtoNoCarrinhoIgualZero) {    
          return setCarrinho(carrinhoAnterior => carrinhoAnterior.filter(itemDoCarrinho => itemDoCarrinho !== produtoNoCarrinhoIgualZero))
        }
